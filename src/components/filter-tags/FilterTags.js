...
function FilterTags() {
  ...
  return (
    <div className="filter-tags__button-container">
      <button onClick={() => setIsOpen(state => !state)} className="filter-tags__current">{searchTag || "All"}</button>
      <ul className={`filter-tags__list ${isOpen && "filter-tags__list_opened"}`}>
        {roleTags.map((el, i) => {
          return (
